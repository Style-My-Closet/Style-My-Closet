...
public class ClothListCacheService {
    ...
    // 옷 목록 첫 페이지 캐시
    @Cacheable(value = "clothListFirstPage", key = "#userId != null ? 'user:' + #userId : 'total'", condition = "#root.target.cachingEnabled")
    public ClothListResponseDto getClothListFirstPage(Long userId, Supplier<ClothListResponseDto> supplier) {
        return supplier.get();
    }

    @CacheEvict(value = "clothListFirstPage", key = "#userId != null ? 'user:' + #userId : 'total'")
    public void evictClothListFirstPage(Long userId) {
    }

    // 속성 목록 첫 페이지 캐시
    @Cacheable(value = "attributeListFirstPage", key = "'first:page'", condition = "#root.target.cachingEnabled")
    public AttributeListResponseDto getAttributeListFirstPage(Supplier<AttributeListResponseDto> supplier) {
        return supplier.get();
    }

    @CacheEvict(value = "attributeListFirstPage", allEntries = true)
